module ApplicationHelper
  ...
  def form_field(form, field_type, field_name, options = {})
    ...
    when :file_field
      form.label(field_name, class: label_class) +
      form.file_field(field_name, class: "block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none")
    when :collection_select
      form.label(field_name, class: label_class) +
      form.collection_select(field_name, options[:collection], options[:value_method], options[:text_method], { prompt: options[:prompt] }, class: input_class)
    else
      ""
    end
