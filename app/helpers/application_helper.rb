module ApplicationHelper
  ...
  def form_field(form, field_type, field_name, options = {})
    ...
    form.label(field_name, class: label_class) + send("form_#{field_type}", form, field_name, options.merge(class: input_class))
  end
  ...
  def form_file_field(form, field_name, options)
    file_input_class = 'block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none'
    form.file_field(field_name, options.merge(class: file_input_class))
  end

  def form_collection_select(form, field_name, options)
    form.collection_select(field_name, options[:collection], options[:value_method], options[:text_method], { prompt: options[:prompt] }, options)
  end
end