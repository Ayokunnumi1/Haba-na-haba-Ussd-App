...
RSpec.describe FoodRequestModule, type: :helpers do
  ...
  let(:selected_district) do District.create!(name: 'Kampala') end
  let(:branch) do Branch.create!(name: 'Haba na Haba Branch', phone_number: '12345678', district_ids: [default_district.id]) end
  let(:kampala_branch) do Branch.create!(name: 'Kampala Branch', phone_number: '123456732', district_ids: [selected_district.id]) end
  ...
  describe '.process_request' do
    context 'when a valid district and branch are found' do
      it 'creates a new food request and sends an SMS' do
        ...
        puts "#{selected_district} district"
        puts "#{kampala_branch} branch"
        text = "1*RequestName*Kampala"
        ...
        expect(request.district_id).to eq(selected_district.id)
